...
# TIMING / METRICS
# ==============================
SETTLE_AFTER_CMD_S = 0.5     # <-- your wait after sending cmd
WINDOW_S = 2.0               # sampling window length
FS_HZ = 500                  # sampling rate
HIGH_THRESH_V = 2.5         # define "high" samples
TOP_N = 10                   # mean of top 20 highs
...
# =========================================================
def run_gate3_termination_check(log_cb=None):
    ...
    log("=" * 60)
    log("[ GATE3] Termination test (peak-based via can_commands)")
    log(f"[ GATE3] settle={SETTLE_AFTER_CMD_S:.1f}s window={WINDOW_S}s fs={FS_HZ}Hz thresh={HIGH_THRESH_V}V topN={TOP_N}")
    ...
    try:
        ...
        # ==================================================
        log("[ GATE3] → Sending TERMINATION_ON")
        termination_on()

        log(f"[ GATE3] Waiting {SETTLE_AFTER_CMD_S:.1f}s before measuring ON...")
        time.sleep(SETTLE_AFTER_CMD_S)

        log("[ GATE3] Measuring ON window...")
        ...
        # ==================================================
        log("[ GATE3] → Sending TERMINATION_OFF")
        termination_off()

        log(f"[ GATE3] Waiting {SETTLE_AFTER_CMD_S:.1f}s before measuring OFF...")
        time.sleep(SETTLE_AFTER_CMD_S)

        log("[ GATE3] Measuring OFF window...")
        ...
        log(f"[ GATE3][ERROR] Unexpected failure: {e}")
        return False
...
if __name__ == "__main__":
    ok = run_gate4_termination_check()
