#!/usr/bin/env python3
"""
RUP ATP Test Platform (1 RUP real-tested, others simulated)

UPDATED (fixes your "Blocked" problem):
- Serial/ID prompt is DISABLED (no QInputDialog anywhere)
- Setup is AUTO-COMPLETED at startup (Quick Test becomes available immediately)
- run_gate_step() is INCLUDED (fixes AttributeError)
- Start ATP button is disabled/locked (because setup is auto)
"""
...
from UI_1RUP.tests.gate3_termination_check import run_gate4_termination_check
from UI_1RUP.tests.gate4_iul_check import run_gate5_iul_check
from UI_1RUP.tests.gate5_ID_check import gate6_id_check
from UI_1RUP.tests.gate6_pd_load import run_gate7
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.setup_locked = True  # we lock because we auto-setup
        ...
        self.timer = QTimer()
        self.timer.timeout.connect(self.run_gate_step)  # MUST exist
        ...
        self.instructions = QLabel(
            "Instructions:\n"
            "1) Insert RUPs in slots\n"
            "2) Click Quick Test (must PASS)\n"
            "3) Then click Full ATP"
        )
        ...
        # Start button exists but will be disabled because we auto-setup
        ...
        # IMPORTANT: auto-setup so Quick Test is not blocked
        self.auto_complete_setup()
    ...
    def open_new_session_files(self):
        ...
        self.log("=== ATP START — IDs DISABLED ===")
        self.log(f"[FILE] Log created: {log_path}")

    def update_title_with_ids(self):
        self.title.setText("RUP ATP — IDs disabled")

    # ================================================================
    # AUTO SETUP (THIS FIXES THE "BLOCKED" ISSUE)
    # ================================================================
    def auto_complete_setup(self):
        self.setup_in_progress = False
        self.setup_complete = True
        self.setup_locked = True

        # Reset gates baseline
        self.gate_results = {g: {1: True, 2: True, 3: True, 4: True} for g in range(1, 8)}
        ...
        self.set_instruction("Setup auto-completed ✅ (IDs disabled)\n\nNext: Click Quick Test.")
        self.log("[UI] Setup AUTO-COMPLETE (IDs disabled)")
        self.set_buttons_state()
    ...
    def set_buttons_state(self):
        ...
        # Start ATP is disabled because we auto-setup
    ...
    # ================================================================
    # (DISABLED) MANUAL SETUP
    # ================================================================
    def start_atp_setup(self):
        QMessageBox.information(
            self,
            "Setup disabled",
            "Serial/ID capture is disabled.\nSetup is auto-completed at startup."
        )
    ...
    def start_quick(self):
        ...
        if not self.setup_complete:
            QMessageBox.warning(self, "Blocked", "Setup not complete.")
            return
    ...
    def run_quick(self):
        if self.current_gate == 0:
            ...
            # Quick only for RUP1, others simulated PASS
            for r in (2, 3, 4):
    ...
    def replace_failed_rups(self):
        ...
        for slot_idx in failed_sorted:
            ...
            msg.setText(f"Remove the failed RUP from Slot {slot_idx}.\n\nClick OK when removed.")
            ...
            ins.setText(f"Insert NEW RUP into Slot {slot_idx}.\n\nClick OK when inserted.")
            ...
            self.log(f"[REWORK] Slot {slot_idx} replaced (IDs disabled)")
    ...
    def write_excel_results(self):
        ...
        path = os.path.join(self.logs_dir, f"ATP_NO_IDS_{ts}.xlsx")
