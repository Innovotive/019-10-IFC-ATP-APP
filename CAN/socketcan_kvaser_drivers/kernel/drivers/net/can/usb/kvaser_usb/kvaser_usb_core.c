...
#define USB_USBCAN_PRO_4HS_SILENT_PRODUCT_ID 0x0118
...
#else
	else if (((struct can_priv *)priv)->ctrlmode & CAN_CTRLMODE_LISTENONLY) {
...
static int kvaser_usb_init_one(struct kvaser_usb *dev, int channel)
{
	...
	if (priv->dev->card_data.capabilities & KVASER_USB_CAP_STATIC_LISTEN_MODE) {
		priv->can.ctrlmode_supported &= ~CAN_CTRLMODE_LISTENONLY;
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 17, 0))
		err = can_set_static_ctrlmode(netdev, CAN_CTRLMODE_LISTENONLY);
		if (err)
			goto candev_free;
#elif (LINUX_VERSION_CODE >= KERNEL_VERSION(4, 7, 0))
		can_set_static_ctrlmode(netdev, CAN_CTRLMODE_LISTENONLY);
#else
#warning Kernel older than v4.7. Devices with static listen-only (always silent), will accept "listen-only off"
		priv->can.ctrlmode_supported |= CAN_CTRLMODE_LISTENONLY;
...
#else
	devlink = devlink_alloc(&kvaser_usb_devlink_ops, sizeof(*dev));
	if (devlink)
		devlink->dev = &intf->dev;
...
#else
	devlink_register(devlink, &intf->dev);
#endif /* LINUX_VERSION_CODE >= KERNEL_VERSION 5.14.0 */
#endif /* (LINUX_VERSION_CODE >= KERNEL_VERSION 5.0.0 && IS_ENABLED(CONFIG_NET_DEVLINK)) || KV_FORCE_DEVLINK */
