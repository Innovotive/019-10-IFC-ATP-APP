...
#define KVASER_USB_LEAF_LEDS_PER_CHANNEL 1
...
static int kvaser_usb_leaf_get_software_info_inner(struct kvaser_usb *dev)
{
	...
	case KVASER_USBCAN:
		...
		dev->fw_version.minor = FIELD_GET(KVASER_USB_SW_VERSION_MINOR_MASK,
						  fw_version);
		dev->fw_version.revision = FIELD_GET(KVASER_USB_SW_VERSION_REVISION_MASK,
						     fw_version);
		dev->max_tx_urbs =
				    ...
				    u16 duration_ms)
{
	...
	cmd->u.led_action_req.action = state |
				       FIELD_PREP(KVASER_USB_LEAF_LED_IDX_MASK,
						  ...
						  KVASER_USB_LEAF_LEDS_PER_CHANNEL * priv->channel);
...
#else
	stats->tx_bytes +=  can_get_echo_skb(priv->netdev, context->echo_index);
