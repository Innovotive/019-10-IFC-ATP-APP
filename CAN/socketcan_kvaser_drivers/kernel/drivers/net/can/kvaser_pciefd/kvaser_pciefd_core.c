...
#error Current kernel version is unsupported. Requires version 4.9 or later.
#endif /* KV_NOT_UPSTREAM */
...
#ifndef netif_txq_completed_wake
static inline void
...
#endif /* ifndef netif_txq_completed_wake */

#ifndef netif_subqueue_completed_wake
...
#endif /* ifndef netif_subqueue_completed_wake */
...
#define KVASER_PCIEFD_KCAN_IO_LED0 BIT(0)
...
static const struct kvaser_pciefd_driver_data kvaser_pciefd_altera_driver_data = {
	...
	.flash_data = &KVASER_PCIEFD_FLASH_DATA_ALTERA,
};
...
static const struct kvaser_pciefd_driver_data kvaser_pciefd_sf2_driver_data = {
	...
	.flash_data = &KVASER_PCIEFD_FLASH_DATA_SF2,
};
...
static const struct kvaser_pciefd_driver_data kvaser_pciefd_xilinx_driver_data = {
	...
	.flash_data = &KVASER_PCIEFD_FLASH_DATA_XILINX,
};
...
{
	if (on)
		can->ioc &= ~KVASER_PCIEFD_KCAN_IO_LED0;
	else
		can->ioc |= KVASER_PCIEFD_KCAN_IO_LED0;
...
#if (LINUX_VERSION_CODE < KERNEL_VERSION(5, 11, 0))
	p->header[1] |= FIELD_PREP(KVASER_PCIEFD_RPACKET_DLC_MASK, can_len2dlc(cf->len));
...
#else
	frame_len = 0;
	can_put_echo_skb(skb, netdev, seq);
					    ...
					    u32 data_len, __le32 *data)
{
	...
	if (p->header[0] & KVASER_PCIEFD_RPACKET_RTR) {
		...
		if (cf->len > data_len * sizeof(*data)) {
			kfree_skb(skb);
			return -EIO;
		}
...
#else
		len = can_get_echo_skb(can->can.dev, echo_idx);
#endif /* LINUX_VERSION_CODE >= 5.12.0) */
...
static int kvaser_pciefd_read_packet(struct kvaser_pciefd *pcie, u32 size, __le32 *buf)
{
	__le64 timestamp;
	...
	if (unlikely(size < KVASER_PCIEFD_PACKET_HDR_LEN))
		return -EIO;

	p->header[0] = le32_to_cpu(*buf++);
	p->header[1] = le32_to_cpu(*buf++);
	...
	case KVASER_PCIEFD_PACK_TYPE_DATA:
		data_len = size - KVASER_PCIEFD_PACKET_HDR_LEN;
		ret = kvaser_pciefd_handle_data_packet(pcie, p, data_len, buf);
		break;
...
{
	__le32 *p = pcie->dma_data[dma_buf],
		*end = p + KVASER_PCIEFD_DMA_SIZE / sizeof(*p);
	u32 size;
	int res = 0;
	unsigned int i;

	for (; (size = le32_to_cpu(*p)); p += size) {
		if (size >= end - p) {
			res = -EIO;
			dev_err_ratelimited(&pcie->pci->dev,
					    "RX overrun (size: %u)\n", size);
			break;
		}

		res = kvaser_pciefd_read_packet(pcie, size - 1, p + 1);
		if (res) {
			dev_err_ratelimited(&pcie->pci->dev,
					    "Corrupt RX buffer: %d\n", res);
			break;
			       ...
			       const struct pci_device_id *id)
{
...
#else
	devlink = devlink_alloc(&kvaser_pciefd_devlink_ops, sizeof(*pcie));
	if (devlink)
		devlink->dev = dev;
	...
	ret = kvaser_pciefd_read_flash_params(pcie);
	if (ret)
		dev_warn(&pcie->pci->dev,
			 "Failed to read flash parameters. "
			 "Continue without EAN and serial_number set.\n");
