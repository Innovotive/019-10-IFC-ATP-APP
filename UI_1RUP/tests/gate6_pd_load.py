...
# ==============================
SINGLE_SLOT = 1  # <<< CHANGE to 2/3/4 if your real RUP is on another slot
...
def _can_send(bus: can.BusABC, slot: int, cmd: int, log, payload=None):
    ...
    if payload is None:
        data = [CMD_PREFIX, cmd]          # DLC=2
    else:
        data = [CMD_PREFIX, cmd] + list(payload)  # DLC=2+len(payload) up to 8

    msg = can.Message(arbitration_id=tx_id, data=data, is_extended_id=False)
    bus.send(msg)
    log(f"📤 CAN TX | slot={slot} id=0x{tx_id:03X} dlc={len(data)} data={[f'0x{b:02X}' for b in data]}")
...
def run_gate6_pd_load(log_cb=None):
    ...
    try:
        log("[GATE7] Starting Gate 7 power check (SINGLE SLOT ONLY)")
        log(f"[GATE7] SINGLE_SLOT={SINGLE_SLOT} (TX_ID=0x{SLOT_TX_ID.get(SINGLE_SLOT, 0x001):03X})")
        ...
        try:
            log(f"[GATE7][DBG] CONSTAT after POWER_TO_60W: {pm.get_connection_status()}")
        except Exception as e:
            log(f"[GATE7][DBG][WARN] get_connection_status failed: {e}")
